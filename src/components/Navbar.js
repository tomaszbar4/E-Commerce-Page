    ...
    return (
        <nav className="flex items-center justify-between p-6 lg:px-0 border-solid border-gray-100 md:border-b-2">
            <div className="flex items-center gap-4">
                ...
                {menuShow && <div className="bg-black/70 transition-all duration-200 absolute left-0 top-0 w-screen lg:hidden z-10 h-full top-0"></div>}
                <div className={menuShow ? "translate-x-0 opacity-1 visible flex flex-col lg:flex-row gap-4 md:gap-8 p-8 ml-4 bg-white h-full w-3/4 z-10 lg:z-0 w-2/3 lg:w-auto lg:h-auto lg:static lg:p-0 -left-4 top-0 absolute text-lg lg:text-medium transition-transform duration-200" : "flex flex-col  lg:visible -translate-x-full lg:translate-x-0 invisible lg:flex-row gap-4 md:gap-8 p-8 ml-4 h-full w-3/4 bg-white lg:z-0 z-20 w-2/3 lg:w-auto lg:h-auto lg:static lg:p-0 -left-4 top-0 absolute text-lg lg:text-medium transition-transform  duration-200"}>
